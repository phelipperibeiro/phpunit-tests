...
use App\Model\Leilao as ModelLeilao;
...
class Leilao
{
    ...
    {
        $sql = 'INSERT INTO leiloes (descricao, dataInicio, finalizado) VALUES (?, ?, ?)';
        $stm = $this->con->prepare($sql);
        $stm->bindValue(1, $leilao->recuperarDescricao(), \PDO::PARAM_STR);
        $stm->bindValue(2, $leilao->recuperarDataInicio()->format('Y-m-d'));
        $stm->bindValue(3, $leilao->estaFinalizado(), \PDO::PARAM_BOOL);
        $stm->execute();
    }
